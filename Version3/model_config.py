...
class DonutDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # CẬP NHẬT: Dùng phép chia lấy dư (%) để lặp lại ảnh an toàn
        ...
        try:
            ...
            # Trả về ảnh trống nếu lỗi để không dừng training
        ...
        try:
            ...
            # Hỗ trợ cả key hoa (Version 3) và key thường (Version 2)
            ...
            # Nếu là Dict lồng nhau (format cũ hơn)
...
def setup_model_and_processor(device):
    ...
    # --- CẬP NHẬT QUAN TRỌNG: KHAI BÁO KÝ TỰ TIẾNG VIỆT LÀ SUB-WORDS ---
    ...
    # Ép Tokenizer KHÔNG được tự động chèn khoảng trắng (lstrip=False, rstrip=False)
    # và KHÔNG coi đây là từ độc lập (single_word=False)
    vietnamese_tokens = [
