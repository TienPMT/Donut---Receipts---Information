...
def run_prediction(model, processor, image):
    ...
    # Lấy ID của token bắt đầu (<s_seller>)
...
def main():
    ...
    # --- SỬA LỖI THIẾU PREPROCESSOR_CONFIG.JSON ---
    try:
        ...
        # Load lại tokenizer từ model cũ để giữ các token tiếng Việt (nếu có)
        try:
