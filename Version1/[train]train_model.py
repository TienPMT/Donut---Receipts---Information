...
def main():
    ...
    train_dataset = DonutDataset(
        dataset_path="data/train_images", # Dữ liệu gốc 1155 ảnh
    ...
    # 3. CẤU HÌNH TỐI ƯU AN TOÀN CHO RTX 5060 (TRÁNH OOM)
    training_args = Seq2SeqTrainingArguments(
        output_dir=OUTPUT_DIR,
        per_device_train_batch_size=2,  # Giảm xuống 2 để an toàn VRAM
        gradient_accumulation_steps=4,  # Tăng lên 4 để giữ Total Batch = 8
        dataloader_num_workers=4,       
        num_train_epochs=50,            
        learning_rate=2e-5,             
        bf16=True,                      
        gradient_checkpointing=True,    # Bật lại để tiết kiệm VRAM tối đa
