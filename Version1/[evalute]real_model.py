...
def main():
    ...
    # Duyệt qua từng ảnh trong thư mục và chạy mô hình
    for img_name in real_images:
        ...
        try:
            ...
            # --- FIX: CHUẨN HÓA KÍCH THƯỚC CHỐNG BÓP MÉO ---
            # Thu phóng tỷ lệ thuật sao cho chiều rộng đúng chuẩn 960px
            # Tránh lỗi chữ bị bóp ngang do camera điện thoại quá rộng
            w, h = image.size
            if w != IMAGE_WIDTH:
                new_h = int(h * (IMAGE_WIDTH / w))
                image = image.resize((IMAGE_WIDTH, new_h), Image.Resampling.LANCZOS)
            
            # Cắt ảnh theo cơ chế cửa sổ trượt chiều cao 1280 chuẩn
            ...
            for crop in crops:
                ...
                with torch.no_grad():
                    outputs = model.generate(
                        ...
                        repetition_penalty=1.5, # <- CHỐNG SA LẦY LOOP CHỮ (Hallucination Loop)
                        no_repeat_ngram_size=5, # <- Chặn lặp một cụm dài quá 5 từ liên tục
                        use_cache=True,
                    )
